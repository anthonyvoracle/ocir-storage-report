#!/usr/bin/env python3
"""
manifest_footprint.py

Estimate image-layer storage footprint from one JSON manifest per image.

Expected input:
  - JSON produced by:
      docker manifest inspect --verbose <image> > image1.json
      docker manifest inspect --verbose <image> > image2.json
    or equivalent OCI/Docker registry manifest JSON.
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Estimate image storage footprint by deduplicating layer digests."
    )
    parser.add_argument(
        "manifests",
        nargs="+",
